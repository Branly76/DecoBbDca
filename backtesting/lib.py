...
def crossunder(series1: Sequence, series2: Sequence) -> bool:
    """
    Return `True` if `series1` just crossed under (above)
    `series2`.
...
class SignalStrategy(Strategy):
    ...
    # adaptions done, replacing scatter parameter as circle parameter. Needed for bokeh version >= 3.4.0
    """
    ...
    def set_signal(self, entry_size: Sequence[float],
        ...
        self.__entry_signal = self.I(  # type: ignore
            ...
            name='entry size', plot=plot, overlay=False, circle=True, color='black')
        ...
        if exit_portion is not None:
            self.__exit_signal = self.I(  # type: ignore
                ...
                name='exit portion', plot=plot, overlay=False, circle=True, color='black')
